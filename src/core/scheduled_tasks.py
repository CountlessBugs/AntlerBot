...
def create_task(
    ...
    trigger: ISO datetime string for once, or cron:EXPR for repeat/complex_repeat.
    source: {"type": "group|private", "id": "chat_id"}.
    ...
    return {"task_id": task["task_id"], "name": unique}
...
async def _recover_missed(tasks: list[dict]) -> list[dict]:
    ...
    for task in tasks:
        ...
        if trigger_str.startswith("cron:"):
            ...
            if run_time < now and task["last_run"] is None:
                missed.append(task)
    ...
    if missed:
        ...
        await agent.invoke(f"以下定时任务在机器人离线期间已到期：\n{lines}")
